...
static void WhereBirdsNest_ApplyArenaPalette(WhereBirdsNestGame *game)
{
    ...
    for (size_t y = 0; y < ARENA_HEIGHT; ++y) {
        for (size_t x = 0; x < ARENA_WIDTH; ++x) {
            ...
            if (character == '#') {
                AsciiCanvas_SetCellColors(&game->canvas, x, y, 0, wallForeground, wallBackground);
            } else if (character == '.') {
                AsciiCanvas_SetCellColors(&game->canvas, x, y, 0, floorForeground, floorBackground);
            }
        }
    }
}

static bool WhereBirdsNest_IsBlocked(const WhereBirdsNestGame *game, float x, float y)
{
    ...
    if (!AsciiCanvas_GetCharacter(&game->canvas, (size_t)cellX, (size_t)cellY, 0, &character, 0)) {
        return true;
    }
...
static bool WhereBirdsNest_CanOccupy(const WhereBirdsNestGame *game, Vector2 position)
{
    return !WhereBirdsNest_IsBlocked(game, position.x - PLAYER_RADIUS,
                                     position.y - PLAYER_RADIUS) &&
           !WhereBirdsNest_IsBlocked(game, position.x + PLAYER_RADIUS,
                                     position.y - PLAYER_RADIUS) &&
           !WhereBirdsNest_IsBlocked(game, position.x - PLAYER_RADIUS,
                                     position.y + PLAYER_RADIUS) &&
...
static bool WhereBirdsNest_OnStart(void *userData, BEngine *engine)
{
    ...
    if (!AsciiCanvas_Init(&game->canvas, ARENA_WIDTH, ARENA_HEIGHT, 1, 0, 0, 18, 1, DARKGREEN)) {
        return false;
    }
    ...
    char assetPath[1024];
    snprintf(assetPath, sizeof(assetPath), "%sassets/wherebirdsnest/arena.txt",
    ...
    WBNCombatTarget_Init(&game->target, (WBNPosition){12.5f, 2.5f}, 3);
...
static void WhereBirdsNest_OnUpdate(void *userData, BEngine *engine, float deltaTime)
{
    ...
    if (BInput_IsActionPressed("basic_attack") && game->attackCooldown <= 0.0f) {
        game->lastAttackResult =
            WBNCombat_TryBasicAttack((WBNPosition){game->playerPosition.x, game->playerPosition.y},
                                     &game->target, BASIC_ATTACK_RANGE, BASIC_ATTACK_DAMAGE);
...
static void WhereBirdsNest_OnRender(void *userData, BEngine *engine)
{
    ...
    AsciiCanvas_DrawCharacter(
        &game->canvas, '@', (Vector2){game->playerPosition.x - 0.5f, game->playerPosition.y - 0.5f},
        GOLD);

    Vector2 targetPosition = {game->target.position.x, game->target.position.y};
    ...
    AsciiCanvas_DrawCharacter(&game->canvas, WBNCombatTarget_IsAlive(&game->target) ? 'D' : '%',
    ...
    if (WBNCombatTarget_IsAlive(&game->target)) {
        DrawText(TextFormat("Target health: %d / %d", game->target.health, game->target.maxHealth),
                 16, 42, 18, LIGHTGRAY);
    } else {
        DrawText("Target defeated", 16, 42, 18, GOLD);
    }

    if (game->attackMessageTime > 0.0f && game->lastAttackResult == WBN_ATTACK_OUT_OF_RANGE) {
