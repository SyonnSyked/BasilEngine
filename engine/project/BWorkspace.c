...
static bool BWorkspaceEntity_ReserveComponents(
    BWorkspaceEntity* entity,
    size_t required,
    BDiagnosticList* diagnostics
)
{
    ...
    if (required > BWORKSPACE_ENTITY_COMPONENT_MAX)
        return BWorkspaceDocument_Fail(diagnostics, BDIAGNOSTIC_INVALID_DATA, "Entity component capacity has been reached.");
    ...
    BWorkspaceComponent* components = (BWorkspaceComponent*)realloc(
        entity->components,
        capacity * sizeof(*components)
    );

    if (components == 0)
        return BWorkspaceDocument_Fail(diagnostics, BDIAGNOSTIC_OUT_OF_MEMORY, "Out of memory while growing an entity component list.");

    memset(
        components + entity->componentCapacity,
        0,
...
{
    if (path == 0 || path[0] == '\0' || strlen(path) >= BWORKSPACE_PATH_MAX ||
        path[0] == '/' || path[0] == '\\' || (isalpha((unsigned char)path[0]) && path[1] == ':'))
    {
        return false;
    }
...
static char* BWorkspaceDocument_ReadFile(const char* path, BDiagnosticList* error)
{
    ...
    if (length < 0 || (uintmax_t)length > SIZE_MAX - 1 ||
        (uintmax_t)length > BWORKSPACE_FILE_MAX || fseek(file, 0, SEEK_SET) != 0)
    {
        fclose(file);
...
)
{
    ...
    for (size_t i = 0; i < source->entityCount; ++i)
    {
        ...
        if (!BWorkspaceEntity_ReserveComponents(cloneEntity, sourceEntity->componentCount, diagnostics))
        {
        ...
        for (size_t componentIndex = 0; componentIndex < sourceEntity->componentCount; ++componentIndex)
        {
            ...
            *cloneComponent = *sourceComponent;
            ...
            if (sourceComponent->kind == BWORKSPACE_COMPONENT_UNKNOWN)
            {
                ...
                if (cloneComponent->data.unknownDataJson == 0)
                {
                    ...
                    clone.entityCount = i + 1;
                    BWorkspaceDocument_Destroy(&clone);
                    return BWorkspaceDocument_Fail(diagnostics, BDIAGNOSTIC_OUT_OF_MEMORY, "Out of memory while cloning component data.");
                }

                memcpy(cloneComponent->data.unknownDataJson, sourceComponent->data.unknownDataJson, length + 1);
            }
...
bool BWorkspaceDocument_Validate(const BWorkspaceDocument* workspace, BDiagnosticList* error)
{
    ...
    for (size_t i = 0; i < workspace->entityCount; ++i)
    {
        ...
        if (entity->componentCapacity > BWORKSPACE_ENTITY_COMPONENT_MAX ||
            ...
            (entity->componentCapacity == 0 && entity->components != 0))
        {
            return BWorkspaceDocument_Fail(error, BDIAGNOSTIC_INVALID_DATA, "Workspace entity component state is invalid.");
        }

        componentTotal += entity->componentCount;

        if (componentTotal > BWORKSPACE_COMPONENT_MAX)
            return BWorkspaceDocument_Fail(error, BDIAGNOSTIC_INVALID_DATA, "Workspace contains too many components.");
        ...
        for (size_t componentIndex = 0; componentIndex < entity->componentCount; ++componentIndex)
        {
            ...
            if (!BWorkspace_IsValidComponentType(component->type) || component->version <= 0)
                return BWorkspaceDocument_Fail(error, BDIAGNOSTIC_INVALID_DATA, "Workspace contains an invalid component envelope.");
            ...
            for (size_t other = componentIndex + 1; other < entity->componentCount; ++other)
            {
                if (strcmp(component->type, entity->components[other].type) == 0)
                    return BWorkspaceDocument_Fail(error, BDIAGNOSTIC_INVALID_DATA, "An entity cannot contain duplicate component types.");
            }

            if (component->kind == BWORKSPACE_COMPONENT_TRANSFORM2D)
            {
                if (strcmp(component->type, BWORKSPACE_TRANSFORM2D_TYPE) != 0 || component->version != 1 ||
                    !isfinite(component->data.transform2d.x) || !isfinite(component->data.transform2d.y))
                {
                    return BWorkspaceDocument_Fail(error, BDIAGNOSTIC_INVALID_DATA, "Transform2D component data is invalid.");
                }
            }
            else if (component->kind == BWORKSPACE_COMPONENT_ASCII_RENDERABLE)
            {
                ...
                if (strcmp(component->type, BWORKSPACE_ASCII_RENDERABLE_TYPE) != 0 || component->version != 1 ||
                    renderable->anchor < BASCII_ANCHOR_BOTTOM_CENTER || renderable->anchor > BASCII_ANCHOR_TOP_LEFT ||
                    renderable->sourceKind < BASCII_SOURCE_GLYPH || renderable->sourceKind > BASCII_SOURCE_TEXT_SPRITE ||
                ...
                {
                    return BWorkspaceDocument_Fail(error, BDIAGNOSTIC_INVALID_DATA, "ASCII Renderable component data is invalid.");
                }
            }
            else if (component->kind == BWORKSPACE_COMPONENT_UNKNOWN)
            {
                ...
                {
                    return BWorkspaceDocument_Fail(error, BDIAGNOSTIC_INVALID_DATA, "Unknown component data is invalid or required.");
                }
            }
            else
            {
                return BWorkspaceDocument_Fail(error, BDIAGNOSTIC_INVALID_DATA, "Workspace component kind is invalid.");
            }
        }
        ...
        {
            return BWorkspaceDocument_Fail(error, BDIAGNOSTIC_INVALID_DATA, "An enabled ASCII Renderable requires Transform2D.");
        }
    }

    if (componentTotal != workspace->componentCount)
        return BWorkspaceDocument_Fail(error, BDIAGNOSTIC_INVALID_DATA, "Workspace component count is inconsistent.");
...
)
{
    ...
    if (document == 0 || component == 0 || entityIndex >= document->entityCount)
        return BWorkspaceDocument_Fail(diagnostics, BDIAGNOSTIC_INVALID_ARGUMENT, "Workspace, entity, and component are required.");

    BWorkspaceEntity* entity = &document->entities[entityIndex];

    if (BWorkspaceEntity_FindComponent(entity, component->type) != 0)
        return BWorkspaceDocument_Fail(diagnostics, BDIAGNOSTIC_INVALID_DATA, "Entity already contains this component type.");
...
)
{
    ...
    {
        BWorkspaceDocument_ClearError(diagnostics);
        return BWorkspaceDocument_Fail(diagnostics, BDIAGNOSTIC_INVALID_ARGUMENT, "ASCII Renderable data is required.");
    }

    if (renderable->anchor < BASCII_ANCHOR_BOTTOM_CENTER || renderable->anchor > BASCII_ANCHOR_TOP_LEFT ||
        renderable->sourceKind < BASCII_SOURCE_GLYPH || renderable->sourceKind > BASCII_SOURCE_TEXT_SPRITE ||
        (renderable->sourceKind == BASCII_SOURCE_GLYPH &&
            ((unsigned char)renderable->glyph < 0x20 || (unsigned char)renderable->glyph > 0x7e)) ||
        (renderable->sourceKind == BASCII_SOURCE_TEXT_SPRITE &&
    ...
    {
        BWorkspaceDocument_ClearError(diagnostics);
        return BWorkspaceDocument_Fail(diagnostics, BDIAGNOSTIC_INVALID_DATA, "ASCII Renderable component data is invalid.");
    }

    if (document == 0 || entityIndex >= document->entityCount ||
        BWorkspaceEntity_FindComponent(&document->entities[entityIndex], BWORKSPACE_TRANSFORM2D_TYPE) == 0)
    {
        BWorkspaceDocument_ClearError(diagnostics);
        return BWorkspaceDocument_Fail(diagnostics, BDIAGNOSTIC_INVALID_DATA, "ASCII Renderable requires an entity with Transform2D.");
    }
...
)
{
    ...
    if (document == 0 || type == 0 || entityIndex >= document->entityCount)
        return BWorkspaceDocument_Fail(diagnostics, BDIAGNOSTIC_INVALID_ARGUMENT, "Workspace, entity, and component type are required.");
    ...
    {
        return BWorkspaceDocument_Fail(diagnostics, BDIAGNOSTIC_INVALID_DATA, "Remove ASCII Renderable before removing its Transform2D.");
    }
    ...
    return BWorkspaceDocument_Fail(diagnostics, BDIAGNOSTIC_INVALID_ARGUMENT, "Entity does not contain the requested component.");
}
...
static bool BWorkspace_ParseComponent(
    const cJSON* source,
    BWorkspaceComponent* destination,
    BDiagnosticList* diagnostics
)
{
    ...
    {
        return BWorkspaceDocument_Fail(diagnostics, BDIAGNOSTIC_INVALID_DATA, "Workspace contains an invalid component envelope.");
    }
    ...
    if (strcmp(destination->type, BWORKSPACE_TRANSFORM2D_TYPE) == 0 && destination->version == 1)
    {
        ...
        if (!BWorkspaceDocument_HasOnlyFields(data, dataFields, 2) ||
            !cJSON_IsNumber(x) || !cJSON_IsNumber(y) || !isfinite(x->valuedouble) || !isfinite(y->valuedouble))
        {
            return BWorkspaceDocument_Fail(diagnostics, BDIAGNOSTIC_INVALID_DATA, "Transform2D component data is invalid.");
        }
    ...
    if (strcmp(destination->type, BWORKSPACE_ASCII_RENDERABLE_TYPE) == 0 && destination->version == 1)
    {
        const char* const dataFields[] = {
            "source", "foreground", "background", "layer", "anchor", "visible", "transparentSpaces"
        };
        ...
        if (!cJSON_IsString(sourceKind))
            return BWorkspaceDocument_Fail(diagnostics, BDIAGNOSTIC_INVALID_DATA, "ASCII Renderable source is invalid.");

        if (strcmp(sourceKind->valuestring, "glyph") == 0)
        {
            cJSON* glyph = cJSON_GetObjectItemCaseSensitive(sourceData, "glyph");

            if (!BWorkspaceDocument_HasOnlyFields(sourceData, glyphFields, 2) || !cJSON_IsString(glyph) ||
                strlen(glyph->valuestring) != 1 || (unsigned char)glyph->valuestring[0] < 0x20 ||
                (unsigned char)glyph->valuestring[0] > 0x7e)
            {
                return BWorkspaceDocument_Fail(diagnostics, BDIAGNOSTIC_INVALID_DATA, "ASCII glyph source is invalid.");
            }
        ...
        else if (strcmp(sourceKind->valuestring, "text-sprite") == 0)
        {
            ...
            if (!BWorkspaceDocument_HasOnlyFields(sourceData, spriteFields, 2) || !cJSON_IsString(path) ||
                !BWorkspace_IsRelativeAssetPath(path->valuestring))
            {
                return BWorkspaceDocument_Fail(diagnostics, BDIAGNOSTIC_INVALID_DATA, "Text Sprite source path is invalid.");
            }

            renderable->sourceKind = BASCII_SOURCE_TEXT_SPRITE;
            snprintf(renderable->textSpritePath, sizeof(renderable->textSpritePath), "%s", path->valuestring);
        }
        else
        {
            return BWorkspaceDocument_Fail(diagnostics, BDIAGNOSTIC_INVALID_DATA, "ASCII Renderable source kind is unsupported.");
        }
        ...
        if (strcmp(anchor->valuestring, "bottom-center") == 0)
            ...
            return BWorkspaceDocument_Fail(diagnostics, BDIAGNOSTIC_INVALID_DATA, "ASCII Renderable anchor is unsupported.");
    ...
    if (destination->required)
        return BWorkspaceDocument_Fail(diagnostics, BDIAGNOSTIC_UNSUPPORTED_VERSION, "A required Workspace component type or version is unsupported.");

    char* unknownJson = cJSON_PrintUnformatted(data);

    if (unknownJson == 0)
        return BWorkspaceDocument_Fail(diagnostics, BDIAGNOSTIC_OUT_OF_MEMORY, "Could not preserve unknown component data.");
    ...
    if (length > BWORKSPACE_UNKNOWN_DATA_MAX)
    {
        ...
        return BWorkspaceDocument_Fail(diagnostics, BDIAGNOSTIC_INVALID_DATA, "Unknown component data exceeds its supported size.");
    }
    ...
    {
        cJSON_free(unknownJson);
        return BWorkspaceDocument_Fail(diagnostics, BDIAGNOSTIC_OUT_OF_MEMORY, "Out of memory while preserving unknown component data.");
    }
    ...
    else
    {
        ...
        for (int i = 0; i < entityCount; ++i)
        {
            ...
            const char* const* entityFields = sourceVersion == BWORKSPACE_SCHEMA_VERSION ?
                currentEntityFields : previousEntityFields;
            ...
            if (sourceVersion == BWORKSPACE_SCHEMA_VERSION)
            {
                ...
                if (componentCount < 0 || componentCount > BWORKSPACE_ENTITY_COMPONENT_MAX ||
                    ...
                    !BWorkspaceEntity_ReserveComponents(destination, (size_t)componentCount, diagnostics))
                {
                    cJSON_Delete(root);
                    workspace.entityCount = (size_t)i + 1;
                    BWorkspaceDocument_Destroy(&workspace);
                    return BWorkspaceDocument_Fail(diagnostics, BDIAGNOSTIC_INVALID_DATA, "Workspace contains too many components.");
                }
                ...
                for (int componentIndex = 0; componentIndex < componentCount; ++componentIndex)
                {
                    ...
                    if (!BWorkspace_ParseComponent(
                        cJSON_GetArrayItem(components, componentIndex),
                        component,
                        diagnostics
                    ))
                    {
                        if (diagnostics != 0 && diagnostics->count > 0)
                        {
                            ...
                            snprintf(diagnostic->entityId, sizeof(diagnostic->entityId), "%s", destination->id);
                            ...
                            if (cJSON_IsString(failedType))
                                snprintf(diagnostic->componentType, sizeof(diagnostic->componentType), "%s", failedType->valuestring);
                        }
    ...
    for (size_t entityIndex = 0; entityIndex < workspace.entityCount; ++entityIndex)
    {
        ...
        for (size_t componentIndex = 0; componentIndex < entity->componentCount; ++componentIndex)
        {
            ...
            if (diagnostics != 0 && diagnostics->count > 0)
            {
                ...
                snprintf(warning->componentType, sizeof(warning->componentType), "%s", component->type);
            }
        }
    }
...
static cJSON* BWorkspace_ComponentDataToJson(
    const BWorkspaceComponent* component,
    BDiagnosticList* diagnostics
)
{
    if (component->kind == BWORKSPACE_COMPONENT_UNKNOWN)
    {
        ...
        {
            cJSON_Delete(data);
            BWorkspaceDocument_Fail(diagnostics, BDIAGNOSTIC_INVALID_DATA, "Preserved unknown component data is no longer valid JSON.");
            return 0;
        }
    ...
    if (data == 0)
    {
        BWorkspaceDocument_Fail(diagnostics, BDIAGNOSTIC_OUT_OF_MEMORY, "Out of memory while serializing component data.");
        return 0;
    }
    ...
    {
        cJSON_Delete(data);
        BWorkspaceDocument_Fail(diagnostics, BDIAGNOSTIC_OUT_OF_MEMORY, "Out of memory while serializing an ASCII source.");
        return 0;
    }
    ...
    const char* anchor = renderable->anchor == BASCII_ANCHOR_CENTER ? "center" :
        renderable->anchor == BASCII_ANCHOR_TOP_LEFT ? "top-left" : "bottom-center";
...
static bool BWorkspaceDocument_WriteJson(const BWorkspaceDocument* workspace, const char* path, BDiagnosticList* error)
{
    ...
    for (size_t i = 0; i < workspace->entityCount; ++i)
    {
        ...
        if (components == 0)
        {
            ...
            cJSON_Delete(root);
            cJSON_Delete(entities);
            return BWorkspaceDocument_Fail(error, BDIAGNOSTIC_OUT_OF_MEMORY, "Out of memory while serializing components.");
        }
