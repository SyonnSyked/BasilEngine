...
{
    return standard == 90 || standard == 99 || standard == 11 ||
        standard == 17 || standard == 23;
}

static bool BProject_IsSupportedCppStandard(int standard)
{
    return standard == 98 || standard == 11 || standard == 14 ||
        standard == 17 || standard == 20 || standard == 23 || standard == 26;
}
...
bool BProject_Validate(const BProject* project, BProjectError* error)
{
    ...
    if (!BProject_IsValidIdentifier(project->identifier))
        return BProject_Fail(error, BPROJECT_ERROR_INVALID_MANIFEST, "Project identifier must be a valid C identifier.");

    if (project->languageMode < BPROJECT_LANGUAGE_C ||
        project->languageMode > BPROJECT_LANGUAGE_MIXED)
    {
        return BProject_Fail(error, BPROJECT_ERROR_INVALID_MANIFEST, "Unknown project language mode.");
    }
...
const char* BProject_LanguageModeToString(BProjectLanguageMode mode)
{
    switch (mode)
    {
        case BPROJECT_LANGUAGE_C: return "c";
        case BPROJECT_LANGUAGE_CPP: return "cpp";
        case BPROJECT_LANGUAGE_MIXED: return "mixed";
...
bool BProject_Load(const char* manifestPath, BProject* outProject, BProjectError* error)
{
    ...
    if (manifestPath == 0 || outProject == 0)
        return BProject_Fail(error, BPROJECT_ERROR_INVALID_ARGUMENT, "Manifest path and output project are required.");
    ...
    if (root == 0)
        return BProject_Fail(error, BPROJECT_ERROR_INVALID_MANIFEST, "Manifest contains invalid JSON.");
    ...
    if (!modeValid)
        return BProject_Fail(error, BPROJECT_ERROR_INVALID_MANIFEST, "Manifest contains an unknown language mode.");
...
bool BProject_Save(const BProject* project, const char* manifestPath, BProjectError* error)
{
    ...
    if (root == 0 || languages == 0)
    {
        ...
        return BProject_Fail(error, BPROJECT_ERROR_IO, "Out of memory while creating the manifest.");
    }
    ...
    cJSON_AddStringToObject(languages, "mode", BProject_LanguageModeToString(project->languageMode));
    ...
    bool succeeded = fwrite(json, 1, length, file) == length &&
        fwrite("\n", 1, 1, file) == 1;
    fclose(file);
    cJSON_free(json);

    if (!succeeded)
        return BProject_Fail(error, BPROJECT_ERROR_IO, "Could not write the complete project manifest.");
