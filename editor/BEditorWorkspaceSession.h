...
class BEditorWorkspaceSession {
  public:
    ...
    bool AddTextSpriteEntity(const std::string &assetId, const std::string &relativePath,
                             std::string &error);
    ...
    bool SetSelectedCollider(BCollider2D collider, std::string &error);
    bool AddSelectedCollider(std::string &error);
    bool RemoveSelectedCollider(std::string &error);

    bool AddSelectedCustomComponent(const std::string &type, int version,
                                    const std::string &dataJson, std::string &error);
