...
bool BEditorUIConfig_Validate(const BEditorUIConfig &config, std::string &error)
{
    ...
    if (config.leftRatio < 0.1f || config.leftRatio > 0.4f || config.rightRatio < 0.1f ||
        config.rightRatio > 0.4f || config.bottomRatio < 0.1f || config.bottomRatio > 0.45f) {
...
bool BEditorUIConfig_Load(const std::string &path, BEditorUIConfig &output, std::string &error)
{
    ...
    if (!stream) {
        error = "Could not open UI Config.";
        return false;
    }
    ...
    if (!cJSON_IsObject(root)) {
        cJSON_Delete(root);
        error = "UI Config contains invalid JSON.";
        return false;
    }
    ...
    bool validTypes = cJSON_IsNumber(schema) &&
                      schema->valuedouble == static_cast<double>(schema->valueint) &&
                      ...
                      ReadBool(root, "showAssets", parsed.showAssets) &&
                      ReadBool(root, "showConsole", parsed.showConsole) &&
                      ReadBool(root, "showBuildOutput", parsed.showBuildOutput) &&
                      ReadBool(root, "showProblems", parsed.showProblems) &&
                      ReadBool(root, "showTerminal", parsed.showTerminal);
    bool supportedSchema = validTypes && (schema->valueint == 1 ||
                                          schema->valueint == BEDITOR_UI_CONFIG_SCHEMA_VERSION);
    ...
    if (showCodeEditor)
        validTypes = validTypes && cJSON_IsBool(showCodeEditor),
        parsed.showCodeEditor = cJSON_IsTrue(showCodeEditor);
    if (!validTypes || !supportedSchema) {
        cJSON_Delete(root);
        error = "UI Config is missing required fields or uses an unsupported schema.";
        return false;
    }
    ...
    cJSON_Delete(root);
    if (!BEditorUIConfig_Validate(parsed, error))
        return false;
    output = parsed;
    return true;
}

bool BEditorUIConfig_Save(const std::string &path, const BEditorUIConfig &config,
                          std::string &error)
{
    if (!BEditorUIConfig_Validate(config, error))
        return false;
    ...
    if (fileError) {
        error = "Could not create UI Config directory: " + fileError.message();
        return false;
    }
    cJSON *root = cJSON_CreateObject();
    if (!root) {
        error = "Out of memory while creating UI Config.";
        return false;
    }
...
#define BASIL_ADD_BOOL(field) cJSON_AddBoolToObject(root, #field, config.field)
    BASIL_ADD_BOOL(showProjectDetails);
    BASIL_ADD_BOOL(showWorkspaceHierarchy);
    BASIL_ADD_BOOL(showInspector);
    BASIL_ADD_BOOL(showWorkspaceViewport);
    BASIL_ADD_BOOL(showAssets);
    BASIL_ADD_BOOL(showConsole);
    BASIL_ADD_BOOL(showBuildOutput);
    BASIL_ADD_BOOL(showProblems);
    BASIL_ADD_BOOL(showTerminal);
...
#undef BASIL_ADD_BOOL
    ...
    cJSON_Delete(root);
    if (!json) {
        error = "Could not serialize UI Config.";
        return false;
    }
    ...
    if (!file) {
        cJSON_free(json);
        error = "Could not open UI Config for writing.";
        return false;
    }
    size_t length = std::char_traits<char>::length(json);
    bool written =
        std::fwrite(json, 1, length, file) == length && std::fwrite("\n", 1, 1, file) == 1;
    bool complete = std::fclose(file) == 0 && written;
    cJSON_free(json);
    if (!complete) {
        fs::remove(temporary, fileError);
        error = "Could not write complete UI Config.";
        return false;
    }
    ...
    if (fs::exists(destination, fileError)) {
        ...
        fileError.clear();
        fs::rename(destination, backup, fileError);
    }
    if (fileError) {
        fs::remove(temporary, fileError);
        error = "Could not back up UI Config.";
        return false;
    }
    ...
    if (fileError) {
        ...
        error = "Could not replace UI Config: " + fileError.message();
        return false;
    }
    error.clear();
    return true;
...
{
    switch (panel) {
        case BEditorPanel::ProjectDetails:
            return "PROJECT DETAILS";
        case BEditorPanel::WorkspaceHierarchy:
            return "WORKSPACE HIERARCHY";
        case BEditorPanel::Inspector:
            return "INSPECTOR";
        case BEditorPanel::WorkspaceViewport:
            return "WORKSPACE VIEWPORT";
        case BEditorPanel::Assets:
            return "ASSETS";
        case BEditorPanel::Console:
            return "STATUS";
        case BEditorPanel::BuildOutput:
            return "BUILD OUTPUT";
        case BEditorPanel::Problems:
            return "PROBLEMS";
        case BEditorPanel::Terminal:
            return "TERMINAL";
        case BEditorPanel::TextSpriteEditor:
            return "TEXT SPRITE EDITOR";
        case BEditorPanel::CodeEditor:
            return "CODE EDITOR";
    }
