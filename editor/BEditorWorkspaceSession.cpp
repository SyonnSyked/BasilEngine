...
bool BEditorWorkspaceSession::Capture(std::vector<std::unique_ptr<Snapshot>> &target,
...
bool BEditorWorkspaceSession::Restore(std::unique_ptr<Snapshot> snapshot, std::string &error)
{
    ...
    selectedIndex_ =
        snapshot->selectedIndex < workspace_.entityCount ? snapshot->selectedIndex : NoSelection;
...
{
    return path_.empty() ? std::filesystem::path{}
                         : std::filesystem::path(path_.string() + ".recovery");
}
...
{
    if (!loaded_ || !dirty_) {
        error = "No modified Workspace is available for recovery.";
        return false;
    }
    BDiagnosticList diagnostics{};
...
bool BEditorWorkspaceSession::HasNewerRecovery() const
{
    ...
    if (!std::filesystem::is_regular_file(recovery, error))
        return false;
...
{
    if (!HasNewerRecovery()) {
        error = "No newer Workspace recovery is available.";
        return false;
    }
...
bool BEditorWorkspaceSession::DiscardRecovery(std::string &error)
{
    ...
    std::filesystem::remove(std::filesystem::path(RecoveryPath().string() + ".bak"), removeError);
    if (removeError) {
        error = "Could not discard Workspace recovery: " + removeError.message();
        return false;
    }
    error.clear();
    return true;
...
bool BEditorWorkspaceSession::SetSelectedName(const std::string &name, std::string &error)
{
    ...
    BDiagnosticList diagnostics{};
    if (selectedIndex_ >= workspace_.entityCount ||
        !BWorkspaceDocument_SetEntityName(&workspace_, selectedIndex_, name.c_str(),
                                          &diagnostics)) {
        CancelMutation();
        error = selectedIndex_ >= workspace_.entityCount ? "No Workspace entity is selected."
...
bool BEditorWorkspaceSession::SetSelectedRenderable(const BAsciiRenderable &renderable,
                                                    std::string &error)
{
    if (selectedIndex_ >= workspace_.entityCount) {
        error = "No Workspace entity is selected.";
    ...
    BDiagnosticList diagnostics{};
    if (selectedIndex_ >= workspace_.entityCount ||
        !BWorkspaceDocument_SetAsciiRenderable(&workspace_, selectedIndex_, &renderable,
                                               &diagnostics)) {
        CancelMutation();
        error = selectedIndex_ >= workspace_.entityCount ? "No Workspace entity is selected."
                                                         : FirstDiagnosticMessage(diagnostics);
...
bool BEditorWorkspaceSession::ValidateForRun(BDiagnosticList &diagnostics, std::string &error) const
{
    if (!loaded_) {
        ...
        BDiagnosticList_Add(&diagnostics, BDIAGNOSTIC_ERROR, BDIAGNOSTIC_INVALID_ARGUMENT,
                            "No Workspace is loaded.", nullptr);
        error = "No Workspace is loaded.";
        return false;
    }
...
bool BEditorWorkspaceSession::DuplicateSelectedEntity(std::string &error)
{
    ...
    if (!BWorkspaceDocument_DuplicateEntity(&workspace_, selectedIndex_, &duplicateIndex,
                                            &diagnostics)) {
        CancelMutation();
        error = FirstDiagnosticMessage(diagnostics);
        return false;
...
bool BEditorWorkspaceSession::AddSelectedCustomComponent(const std::string &type, int version,
                                                         ...
                                                         std::string &error)
{
    ...
    if (!Capture(undo_, error))
        return false;
    BDiagnosticList diagnostics{};
    if (!BWorkspaceDocument_AddCustomComponentJson(&workspace_, selectedIndex_, type.c_str(),
                                                   version, dataJson.c_str(), &diagnostics)) {
        CancelMutation();
        error = FirstDiagnosticMessage(diagnostics);
        return false;
    }
...
bool BEditorWorkspaceSession::RemapAssetPath(const std::string &oldPath, const std::string &newPath,
                                             std::string &error)
{
    if (!loaded_ || oldPath.empty() || newPath.empty() || newPath.size() >= BWORKSPACE_PATH_MAX) {
        error = "Valid old and new Project asset paths are required.";
        return false;
    }
...
bool BEditorWorkspaceSession::CanUndo() const
{
    return !undo_.empty();
}
bool BEditorWorkspaceSession::CanRedo() const
{
    return !redo_.empty();
}

bool BEditorWorkspaceSession::IsLoaded() const
{
    return loaded_;
}
bool BEditorWorkspaceSession::IsDirty() const
{
    return dirty_;
}
...
std::uint64_t BEditorWorkspaceSession::Revision() const
{
    return revision_;
}
void BEditorWorkspaceSession::MarkDirty()
{
    if (loaded_) {
        CommitMutation();
    }
}
const std::filesystem::path &BEditorWorkspaceSession::Path() const
{
    return path_;
}
...
const BWorkspaceDocument &BEditorWorkspaceSession::Workspace() const
{
    return workspace_;
}
BWorkspaceDocument &BEditorWorkspaceSession::MutableWorkspace()
{
    return workspace_;
}
std::size_t BEditorWorkspaceSession::SelectedIndex() const
{
