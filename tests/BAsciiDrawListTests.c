...
static bool AddRenderableEntity(BWorkspaceDocument *document, const char *name,
                                BTransform2D transform, const BAsciiRenderable *renderable,
...
int main(void)
{
    ...
    failures += Check(BWorkspaceDocument_CreateDefault(&document, "Main", "Main", &diagnostics),
                      "Workspace is created");

    BAsciiRenderable highGlyph = BAsciiRenderable_DefaultGlyph('@');
    highGlyph.layer = 5;
    highGlyph.foreground = (BAsciiColor){1, 2, 3, 4};
    failures += Check(AddRenderableEntity(&document, "High", (BTransform2D){10.25f, 20.5f},
                                          &highGlyph, NULL, &diagnostics),
    ...
    failures += Check(AddRenderableEntity(&document, "Sprite", (BTransform2D){4.0f, 8.0f}, &sprite,
                                          NULL, &diagnostics),
    ...
    failures += Check(AddRenderableEntity(&document, "Same layer", (BTransform2D){-1.0f, -2.0f},
                                          &sameLayerGlyph, NULL, &diagnostics),
    ...
    failures += Check(AddRenderableEntity(&document, "Top left", (BTransform2D){100.0f, 200.0f},
                                          &topLeftSprite, NULL, &diagnostics),
                      "top-left Text Sprite entity is added");

    BAsciiRenderable hidden = BAsciiRenderable_DefaultGlyph('H');
    size_t disabledIndex = 0;
    failures += Check(AddRenderableEntity(&document, "Disabled", (BTransform2D){0.0f, 0.0f},
                                          &hidden, &disabledIndex, &diagnostics),
                      "disabled fixture entity is added");
    document.entities[disabledIndex].enabled = false;
    hidden.visible = false;
    failures += Check(AddRenderableEntity(&document, "Invisible", (BTransform2D){0.0f, 0.0f},
                                          &hidden, NULL, &diagnostics),
                      "invisible fixture entity is added");
    failures += Check(BWorkspaceDocument_AddEntity(&document, "Empty", NULL, &diagnostics),
                      "empty entity is valid");

    BTextSpriteCache cache;
    BTextSpriteCache_Init(&cache);
    BAsciiDrawList list;
    BAsciiDrawList_Init(&list);
    failures += Check(BAsciiDrawList_Build(&document, root, &cache, &list, &diagnostics),
                      "draw list builds");
    failures +=
        Check(list.count == 8, "transparent sprite spaces and suppressed entities emit no items");
    failures += Check(list.items[0].glyph == 'A' && list.items[1].glyph == 'B' &&
                          list.items[2].glyph == 'C' && list.items[3].glyph == 'Z' &&
                          list.items[4].glyph == 'A' && list.items[7].glyph == '@',
                      "items sort by layer, entity order, and sprite row/column order");
    failures +=
        Check(list.items[0].x == 3.5f && list.items[0].y == 7.5f && list.items[1].x == 3.5f &&
                  list.items[1].y == 8.5f && list.items[2].x == 4.5f && list.items[2].y == 8.5f,
              "even centered Text Sprite receives fractional anchor placement");
    failures += Check(list.items[4].x == 100.0f && list.items[4].y == 200.0f &&
    ...
    failures += Check(strcmp(list.items[0].entityId, document.entities[1].id) == 0 &&
    ...
    failures += Check(!BAsciiDrawList_Build(&document, root, &cache, &list, &diagnostics),
                      "malformed asset rejects interpretation");
    const BDiagnostic *error = BDiagnosticList_FirstError(&diagnostics);
    failures += Check(error != NULL && strcmp(error->entityId, document.entities[1].id) == 0 &&
                          strcmp(error->componentType, BWORKSPACE_ASCII_RENDERABLE_TYPE) == 0,
                      "asset error identifies its entity and component");
    failures += Check(list.count == retainedCount && list.items[0].glyph == retainedGlyph,
                      "failed build preserves prior draw list");
    const BTextSprite *retainedSprite = BTextSpriteCache_Find(&cache, "assets/ship.txt");
    failures += Check(retainedSprite != NULL && retainedSprite->width == 2,
                      "failed refresh preserves cached asset");

    failures += Check(WriteText(spritePath, "A \nBC\n"), "valid LF refresh is restored");
    BWorkspaceComponent *spriteComponent =
        BWorkspaceEntity_FindComponent(&document.entities[1], BWORKSPACE_ASCII_RENDERABLE_TYPE);
    ...
    failures += Check(BAsciiDrawList_Build(&document, root, &cache, &list, &diagnostics),
                      "opaque-space bottom-center list builds");
    failures += Check(list.count == 9 && list.items[1].glyph == ' ',
                      "nontransparent spaces remain explicit draw items");
    failures += Check(list.items[0].x == 3.5f && list.items[0].y == 7.0f,
                      "bottom-center anchor uses the bottom row as origin");

    BWorkspaceDocument empty;
    BWorkspaceDocument_Init(&empty);
    failures += Check(BWorkspaceDocument_CreateDefault(&empty, "Empty", "Empty", &diagnostics),
                      "empty Workspace is created");
    failures += Check(BAsciiDrawList_Build(&empty, root, &cache, &list, &diagnostics),
                      "empty Workspace builds successfully");
