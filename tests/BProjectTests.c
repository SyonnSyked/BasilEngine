...
int main(void)
{
    ...
    failures += Check(project.languageMode == BPROJECT_LANGUAGE_MIXED,
                      "default project uses mixed languages");
    ...
    failures += Check(strcmp(project.startupWorkspace, "workspaces/Main.basilworkspace") == 0,
    ...
    failures += Check(error.code == BPROJECT_ERROR_INVALID_MANIFEST,
                      "validation returns a useful error code");

    invalid = project;
    snprintf(invalid.startupWorkspace, sizeof(invalid.startupWorkspace),
             "../Outside.basilworkspace");
    failures +=
        Check(!BProject_Validate(&invalid, &error), "escaping startup Workspace path is rejected");

    BProject loaded;
    failures +=
        Check(!BProject_Load("test-fixtures/project-invalid-json.basilproject", &loaded, &error) &&
    ...
    failures +=
        Check(BProject_Load("test-fixtures/project-legacy-version.basilproject", &loaded, &error),
              "legacy project manifest loads compatibly");
    failures += Check(loaded.schemaVersion == BPROJECT_SCHEMA_VERSION,
                      "legacy manifest is represented using the current schema");
    failures += Check(strcmp(loaded.startupWorkspace, "scenes/Main.scene") == 0,
                      "legacy startup path is preserved until explicit migration");

    char uniqueIdentifier[BPROJECT_IDENTIFIER_MAX];
    snprintf(uniqueIdentifier, sizeof(uniqueIdentifier), "Generated_%ld_%d", (long)time(0),
             (int)GET_PROCESS_ID());
    project = BProject_Default("Generated Test Project", uniqueIdentifier);

    failures +=
        Check(BProjectGenerator_Create(&project, ".", &error), "project generator succeeds");
    ...
    char inputPath[BPROJECT_PATH_MAX];
    char cppPath[BPROJECT_PATH_MAX];
    char workspacePath[BPROJECT_PATH_MAX];
    char assetRegistryPath[BPROJECT_PATH_MAX];
    ...
    snprintf(inputPath, sizeof(inputPath), "%s/.basil/input.json", root);
    ...
    snprintf(assetRegistryPath, sizeof(assetRegistryPath), "%s/.basil/assets.json", root);
    ...
    failures += Check(FileExists(assetRegistryPath), "generator writes versioned asset registry");
    failures += Check(!BProjectGenerator_Create(&project, ".", &error) &&
                          error.code == BPROJECT_ERROR_ALREADY_EXISTS,
                      "generator refuses to overwrite an existing project");

    BAssetRegistry generatedRegistry;
    BAssetRegistry_Init(&generatedRegistry);

    BDiagnosticList assetDiagnostics;

    failures += Check(BAssetRegistry_Load(assetRegistryPath, &generatedRegistry, &assetDiagnostics),
                      "generated asset registry loads");

    failures += Check(generatedRegistry.schemaVersion == BASSET_REGISTRY_SCHEMA_VERSION,
                      "generated asset registry uses current schema");

    failures += Check(generatedRegistry.count == 0, "generated asset registry starts empty");

    BAssetRegistry_Destroy(&generatedRegistry);
    ...
    failures +=
        Check(strcmp(loaded.identifier, project.identifier) == 0, "loaded identifier matches");
    ...
    failures += Check(strcmp(loaded.startupWorkspace, project.startupWorkspace) == 0,
